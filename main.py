...
class App(tk.CTk):
    ...
    def __init__(self):
        ...
        self.label = tk.CTkLabel(master=self, text="1. Wybierz walutę", width=120, height=25, fg_color="#57c1fa",
                                 corner_radius=8)
        self.label.grid(row=0, column=3)
        self.createRadioButtons()

        self.label12 = tk.CTkLabel(master=self, text="2. Wybierz datę początkową", width=120, height=25,
    ...
    def changeString(self, string):
        ...
        nstring1 = '20'+nstring[2]+'-'+(nstring[0] if len(nstring[0])==2 else '0'+nstring[0])+'-'+(nstring[1] if len(nstring[1])==2 else '0'+nstring[1])
        return nstring1
    def plotGraph(self, rates, dates):
        ...
        canvas.draw()
        canvas.get_tk_widget().grid(row=9, column=0, columnspan=5, padx=25, pady=25)
    def getRates(self):
        response = requests.get(f"http://api.nbp.pl/api/exchangerates/rates/A/{self.selected_currency.get()}/{self.changeString(self.cal.get_date())}/{self.changeString(self.cal1.get_date())}/")
